...
export default function SemanticAnalysis({
  ...
  const allFamilies = [
    ...new Set([
      ...seq1Motifs.map((m) => m.family),
      ...seq2Motifs.map((m) => m.family),
    ]),
  ];
    ...
    <div className="space-y-6">
      <div>
        <h4 className="font-medium mb-3">
          Transcription Factor Family Distribution
        </h4>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {allFamilies.map((family) => {
            const seq1Count = seq1Motifs.filter(
              (m) => m.family === family
            ).length;
            const seq2Count = seq2Motifs.filter(
              (m) => m.family === family
            ).length;
            return (
              <div
        ...
        <div>
          ...
          <Alert className="border-teal-600 bg-teal-50 text-teal-600 ">
            ...
            <AlertDescription>
              ...
              families, suggesting potential functional conservation through
              redundancy.
            </AlertDescription>
          </Alert>
      ...
      <div className="p-4 bg-gray-50 rounded-lg">
        ...
        <ul className="text-sm space-y-1 text-gray-700">
          <li>
            • TFs from the same family often have similar DNA-binding
            preferences
          </li>
