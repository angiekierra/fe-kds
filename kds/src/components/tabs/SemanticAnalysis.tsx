      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {allFamilies.map((family) => {
            ...
            return (
              <div key={family} className="p-3 border border-gray-300 rounded-lg">
                <div className="flex justify-between items-center mb-2">
                  <span className="font-medium">{family}</span>
                  <Badge variant="outline" className="bg-amber-100 border-amber-500 text-amber-600 rounded-full">{seq1Count + seq2Count} total</Badge>
                </div>
          ...
          <div className="mt-3 space-y-2">
            {Object.entries(familyMatches).map(([family, matches]) => (
              <div key={family} className="p-3 bg-violet-50 rounded-lg">
                <div className="font-medium text-violet-900">{family} Family</div>
                <div className="text-sm text-violet-700 mt-1">Cross-sequence matches: {matches.join(", ")}</div>
