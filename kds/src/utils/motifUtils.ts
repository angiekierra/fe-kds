...
export const MOTIF_DEFINITIONS_MAP: Record<
  string,
  MotifDefinition & { pwm: PWM }
> = {};
...
export function convertPfmToPwm(pfm: PFM): PWM {
  ...
  for (let i = 0; i < motifLength; i++) {
    ...
    pwm.A.push(
      totalCountsAtPosition > 0 ? pfm.A[i] / totalCountsAtPosition : 0
    );
    pwm.C.push(
      totalCountsAtPosition > 0 ? pfm.C[i] / totalCountsAtPosition : 0
    );
    pwm.G.push(
      totalCountsAtPosition > 0 ? pfm.G[i] / totalCountsAtPosition : 0
    );
    pwm.T.push(
      totalCountsAtPosition > 0 ? pfm.T[i] / totalCountsAtPosition : 0
    );
  }
  return pwm;
}

export function normalizePWMScore(
  rawScore: number,
  minScore: number,
  maxScore: number
): number {
  ...
  for (let i = 0; i < motifLength; i++) {
    ...
    const bgProb =
      backgroundFrequencies[
        nucleotide as keyof typeof DEFAULT_BACKGROUND_FREQUENCIES
      ];
...
export function findMotifs(sequence: string, tss: number): MotifMatch[] {
  ...
  Object.values(MOTIF_DEFINITIONS_MAP).forEach((unifiedDef) => {
    ...
    foundPwmMatches.forEach((match) => {
      ...
      const distanceFromCanonical = Math.abs(
        relativePosition - unifiedDef.canonical_position
      );
      const compatibility_score = Math.max(0, 1 - distanceFromCanonical / 100);

      const familyName =
        unifiedDef.family && unifiedDef.family.length > 0
          ? unifiedDef.family[0]
          : "Unknown";
